...
contract RandomRain is ERC721, Ownable {
    ...
    constructor(address _renderer) ERC721("Random Rain", "RAIN") Ownable(msg.sender) {
    ...
    function preview(uint256 seed) external view returns (string memory) {
        string memory svgData = renderer.svg(seed);
        string memory htmlData = renderer.html(seed, true);
